...
```
Ja, de finns i intervallen. Det är förväntat resultat, eftersom vardera parameter bör ligga inom sitt intervall i ca 95 av 100 upprepade experiment.
...
Diagnostikplottar varierar avsevärt mellan upprepade simuleringar vid små n. Relativt stabilt är däremot hävstångsvärdena (de beror på x-värdena) och Cook’s konturlinjer (de beror på n och antalet parametrar). Vilka punkter som får stor Cook’s-distans skiftar med residualerna. I QQ-plottar är mitten ofta ganska stabil medan svansarna varierar mer.
...
* **Residuals vs Leverage:** minst en punkt ligger kring/över Cook’s 0,5 ⇒ potentiellt inflytelserik, bör granskas.
...
* **Inflytelserik outlier (fall 2):** Punkt med **extremt x** drar regressionslinjen (synligt i “Med anpassning”). I **Residuals vs Leverage** har den **hög hävstång** och ligger nära/över **Cook’s**-konturer ⇒ **inflytelserik**. Residualen är inte nödvändigtvis störst i “Residuals vs Fitted”, men kombinationen av hög leverage + stort risidual gör den tydligt påverkande.
...
### Tolkning 2(b) – Multipel regression (CO \~ tar + nico + weight)

**Lutningskoefficienter jämfört med (a):**

* **tar:** β ≈ 0.96 (p ≈ 0.0007) i multipel modellen, jämfört med β ≈ 0.80 i enkel. Fortsatt **positiv och signifikant** – i linje med (a), och något starkare när vi kontrollerar för övriga variabler.
* **nico:** β går från **+12.40 (starkt signifikant)** i enkel till **−2.63 (icke-signifikant, p ≈ 0.51)** i multipel. Detta **teckenbyte** tyder på stark **kollinearitet** med tar: den positiva effekt som syntes i enkel regression var till stor del gemensam med tar och försvinner när tar hålls konstant.
* **weight:** β ≈ +25.07 (svagt stöd, p ≈ 0.02) i enkel → **≈ −0.13 (p ≈ 0.97)** i multipel. Effekten försvinner helt när tar/nico kontrolleras, vilket också är förenligt med kollinearitet/överlapp i förklarad variation.

**p-värden:**
Endast **tar** är signifikant i den multipla modellen. Både **nico** och **weight** tappar signifikans (från \*\*\* respektive \* i de enkla modellerna till ns), vilket är typiskt när prediktorer är starkt samvarierade och de unika effekterna blir små/instabila.

**Förklaringsgrad (R² och justerat R²):**

* **Multipel modell:** R² = **0.9186**, justerat R² = **0.9070**.
* **Jämförelse med enkla modeller:**

  * CO \~ **tar**: R² = **0.9168**, justerat R² = **0.9132**.
  * CO \~ **nico**: R² = **0.8574**, justerat R² = **0.8512**.
  * CO \~ **weight**: R² = **0.2153**, justerat R² = **0.1811**.

**Vad noterar vi?**

* R² ökar endast **marginellt** när nico och weight läggs till (0.9168 → 0.9186), medan **justerat R² sjunker** (0.9132 → 0.9070) eftersom extra, onödiga variabler straffas.
* **Residual standard error** ökar något (≈ **1.40** i CO\~tar → **1.45** i multipel), vilket inte talar för en bättre modell.
* Sammantaget ger den multipla modellen **ingen praktisk förbättring** jämfört med den enkla modellen **CO \~ tar**. Teckenbytet för nico och bortfallet av weight-effekten är förenliga med **multikollinearitet** mellan prediktorerna.
* **Rekommendation:** För dessa data är den **enklare modellen med endast tar** att föredra (bäst justerat R², enklast tolkning, stabila koefficienter).
...
**tar–nico** ligger nästan helt på en linje ⇒ mycket stark **kollinearitet**. **CO** korrelerar starkt med både **tar** och **nico**, båda förklarar CO bra var för sig. Medan **weight** bara har ett svagt samband med CO men ett svagt–måttligt positivt med tar/nico.

Följden blir att **nico** och **weight** nästan inte tillför någon **unik** förklaringskraft när **tar** finns i modellen. Nicos positiva enkla effekt ”äts upp” av den starka samvariationen med tar och till och med byter tecken (suppressoreffekt). Vikteffekten verkar främst gå via högre tar/nico i tyngre cigaretter; när dessa hålls konstanta försvinner viktsambandet. Därför ökar R² knappt, justerat R² sjunker och endast **tar** kvarstår som signifikant — tar fångar i praktiken nästan all relevant variation i CO.
...
* **Residuals vs Leverage:** observation **3** har **hög hävstång** och **stort residual** (nära/över Cook’s) ⇒ **inflytelserik**.

Den multipla linjära modellen är delvis rimlig men diagnosen visar möjlig icke-linearitet, icke-konstant varians och en inflytelserik punkt (nr 3) som bör utredas/hanteras.
...
Om observationen tas bort bör man presentera en kort känslighetsanalys: visa båda modellerna sida vid sida (med/utan observationen) i en tabell med β-koefficienter, standardfel, p-värden, R²/justerat R² och en kort kommentar som motiverar exkluderingen.


Slutsatsen: CO förklaras främst av **tjära**; outlieren påverkade koefficienterna (framför allt tecknet för nikotin) och försämrade passformen.
...
För prediktion av CO är den enkla modellen $\mathrm{CO} \sim \mathrm{tar}$ bäst. Den identifierade inflytelserika observationen tas bort eftersom den kraftigt påverkar skattningarna; exkludering ger lägre prediktionsfel (LOOCV-RMSEP) och högre justerat $R^2$, dvs. en mer stabil modell. Samtidigt är evidensen för ett starkt positivt samband mellan CO och tjära robust och kvarstår även när observationen inkluderas. Nikotin är starkt kollinear med tjära och blir därför överflödig i den multipla modellen, medan vikt tillför lite. 

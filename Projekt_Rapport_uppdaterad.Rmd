...
Sambanden mellan tillförsel av löst oorganiskt kväve (DIN) och demografiska- samt olika mark faktorer i Östersjöns avrinningsområden har analyserats med en förklaringsmodell som vi tagit fram genom logtransformeringar, manuell variabelselektion och step-funktionen. Resultaten i vår modell visar att DIN främst ökar med befolkning, avrinning och antal svin, medan hög anslutning till reningsverk minskar DIN. Sverige har lägre grundnivå än övriga länder men ingen statistiskt signifikant skillnad mellan förklaringsvariablerna. Även en prediktionsmodell togs fram med relativt RMSEP på 34% vilket i vårt sammanhang får anses bra.

# 2 Inledning

Målet är att modellera och jämföra hur DIN (dissolved inorganic nitrogen) i tillrinnande vattendrag påverkas av avrinningsområdets storlek, befolkning, demografi, marksammansättning, djurhållning, avrinning samt reningsanslutning. Samt att bedöma om sambanden skiljer mellan Sverige och övriga länder. Vi utvärderar modeller både för inferens och prediktion (LOOCV).

# 3 Data

**Källa och struktur.** Datasetet `baltic_DIN.csv` består av avrinningsområden med variabler $x_1,\dots,x_{20}$ och respons $y$ (DIN, kg). Vi omvandlar areor från ha till km², logtransformerar vissa variabler och gör om areor till andelar. Samt omvandlar x20 till en faktor för att kunna använda i regressionsmodell.
...
Vi ser att Litauen har en observation och Polen och Ryssland har endast 2. 
>>>>>>> Stashed changes
...
Spridningsdiagrammen ($y$ mot $x_1$–$x_{19}$) visar tydligt positiva samband för $x_1$ (avrinningsområdets area), $x_2$ (befolkning), $x_{11}$ (jordbruksmark), $x_{15}$ (artificiella ytor), $x_{17}$ (svin) och $x_{18}$ (nöt); $x_9$ (buskvegetation) ser också positiv ut men effekten drivs av några få extrema värden, och $x_{13}$ (inlandsvatten) är svagt–måttligt positiv. Sambanden är svaga eller oklara för $x_3$ (urban andel), $x_5$ (befolkningstäthet), skogstyperna $x_6$–$x_8$ samt $x_{10}$ (våtmark). Negativ lutning ses för $x_{12}$ (häll/berg), $x_{14}$ (snö/is) och $x_{19}$ (reningsanslutning), medan $x_{16}$ (avrinning) ligger nära noll eller svagt negativ i råa data. Variationen i $y$ ökar med nivån och flera paneler påverkas av några mycket stora vattendrag (heteroskedasticitet och outliers). Boxplottar av $y$ per land visar Polen avvikande med sina 2 observationer, Ryssland och Lettland mellanhöga samt Sverige och Finland generellt låga nivåer. Histogrammet visar att $y$ är kraftigt högerskev med några extrema toppar, vilket motiverar en $\log$-transform av $y$; dessutom ligger många punkter långt till vänster i plottarna, vilket ytterligare talar för $\log$-transform för att få en mer symmetrisk fördelning.
>>>>>>> Stashed changes

# 4 Statistisk modellering

**Kommentar om kollinearitet.**
Vi har $x3+x4=100$ (perfekt kollinearitet) samt $x5= x2 / x1$. Summan av markandelsvariabler $x6$–$x15$ är $x1$, vilket ger nära-perfekt kollinearitet. Vi väljer därför att exkludera $x4$, $x5$ och $x12$ för att minska multikolineariteten.

Vi börjar med att transformera responssvariabeln $y$ och $x_1$, $x_2$, $x_17$ och $x_18$. På variablarna $x6-x15$ tar vi proportionerna för den observationen, alltså delad med $x1$. Vi testade ett ny scatterplots, histogram och lådagram. 
...
```
```{r}
print(paste("Relativt prediktionsfel för modell A:", rmse_A / sd(df$y)))
print(paste("Relativt prediktionsfel för modell B:", rmse_B / sd(df$y)))
print(paste("Relativt prediktionsfel för modell C:", rmse_C / sd(df$y)))
```
Relativt prediktionsfel för modell A = 0.342 säger att felet i prediktionerna är ca 34% av standardavvikelsen för y, vilket ses som ett bra värde för en prediktionsmodell.
